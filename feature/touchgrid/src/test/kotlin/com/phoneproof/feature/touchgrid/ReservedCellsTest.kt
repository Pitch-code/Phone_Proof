...
class ReservedCellsTest {
    ...
    fun `no insets reserves nothing`() {
        assertTrue(reservedCells(spec, width = 1000, height = 1000).isEmpty())
    }
    ...
    fun `a zero sized canvas reserves nothing rather than everything`() {
        ...
        // tested" at the buyer.
        assertTrue(reservedCells(spec, width = 0, height = 0, top = 50).isEmpty())
        assertTrue(reservedCells(spec, width = 1000, height = 0, top = 50).isEmpty())
    }
    ...
    fun `a top inset reserves whole rows across the full width`() {
        ...
        // not strictly less than 150, so only row 0 is reserved.
        val reserved = reservedCells(spec, width = 1000, height = 1000, top = 150)

        assertEquals((0 until 10).map { Cell(it, 0) }.toSet(), reserved)
    }

    @Test
    fun `a cell is reserved once its centre is covered, not on first overlap`() {
        // 151px passes the centre of row 1 (150px), so rows 0 and 1 both go.
        val reserved = reservedCells(spec, width = 1000, height = 1000, top = 151)
        val expected = (0 until 10).flatMap { column ->
            listOf(Cell(column, 0), Cell(column, 1))
        }.toSet()

        assertEquals(expected, reserved)
    }

    @Test
    fun `an inset thinner than half a cell reserves nothing`() {
        // 49px does not reach row 0's centre at 50px. Treating any overlap as reserved would write
        // off an entire row of testable screen for a sliver of inset.
        assertTrue(reservedCells(spec, width = 1000, height = 1000, top = 49).isEmpty())
    }

    @Test
    fun `a bottom inset reserves rows measured from the far edge`() {
        val reserved = reservedCells(spec, width = 1000, height = 1000, bottom = 150)

        assertEquals((0 until 10).map { Cell(it, 9) }.toSet(), reserved)
    }

    @Test
    fun `side insets reserve whole columns down the full height`() {
        val reserved = reservedCells(spec, width = 1000, height = 1000, left = 60, right = 60)
    ...
    fun `all four insets combine into a frame`() {
        ...
        assertEquals(36, reserved.size)
        assertTrue(Cell(0, 0) in reserved)
        assertTrue(Cell(5, 0) in reserved)
        assertTrue(Cell(0, 5) in reserved)
        assertTrue(Cell(9, 9) in reserved)
        assertTrue(Cell(5, 5) !in reserved)
    }

    @Test
    fun `insets larger than the canvas reserve every cell without crashing`() {
        val reserved = reservedCells(spec, width = 1000, height = 1000, top = 5000)

        assertEquals(spec.cellCount, reserved.size)
    }
    ...
    fun `the real grid and a realistic phone reserve only the edges`() {
        ...
        assertTrue(reserved.size < GridSpec.Default.cellCount / 3)
        assertTrue(Cell(8, 0) in reserved)
        assertTrue(Cell(8, 31) in reserved)
        assertTrue(Cell(0, 16) in reserved)
        assertTrue(Cell(8, 16) !in reserved)
    }
}
