...
These are Play Console fields, not app code, so they live here rather than in `strings.xml`.

| Field | Value |
| --- | --- |
| **Title** (30 char limit) | `PhoneProof: Test Any Phone` — 26 characters |
| **Icon** (512×512 PNG) | `screenshots/play-store-icon-512.png`, generated by CI from the shipped drawable |

The title is **not** the same thing as the name under the icon on a home screen. That one is
`app_name` in `app/src/main/res/values/strings.xml` and stays **`PhoneProof`**, because launchers
truncate at roughly eleven characters and `PhoneProof: Test Any P…` looks broken.

The chosen title was `PhoneProof: Used/new Phone Test` until it was counted: 31 characters, one over
Play's limit. `Test Any Phone` covers used and new without the slash, and keeps the two words people
actually search for.

Do not hand-make the 512 icon. It is rendered from `ic_launcher_foreground`, which is what guarantees
the store listing and the installed app show the same mark — they are seen side by side the moment
someone installs, and differing is both obvious and avoidable.
